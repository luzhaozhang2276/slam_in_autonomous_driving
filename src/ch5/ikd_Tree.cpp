...
KD_TREE<PointType>::KD_TREE(float delete_param, float balance_param, float box_length) {
    ...
    std::cout << "kd-tree init" << std::endl;
...
int KD_TREE<PointType>::validnum() {
    int s = 0;
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
        if (Root_Node != nullptr) return (Root_Node->TreeSize - Root_Node->invalid_point_num);
        else
            return 0;
    } else {
        if (!pthread_mutex_trylock(&working_flag_mutex)) {
...
bool KD_TREE<PointType>::GetClosestPointMT(const PointVector &cloud, std::vector<std::pair<size_t, size_t>> &matches,
    ...
    std::for_each(std::execution::par_unseq, index.begin(), index.end(), [this, &cloud, &matches, &k](int idx) {
        PointVector k_indices;
        std::vector<float> k_sqr_distances;
        Nearest_Search(cloud[idx], k, k_indices, k_sqr_distances);
        int n = static_cast<int>(k_indices.size());
        for (int i = 0; i < k; ++i) {
            matches[idx * k + i].second = idx;
            if (i < n) {
                matches[idx * k + i].first = k_indices[i].intensity;
            } else {
